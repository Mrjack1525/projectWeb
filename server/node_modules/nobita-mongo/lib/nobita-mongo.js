...
const create = (databaseName, tableName, schema) => {
  ...
  _f[tableName] = {
    ...
    async createMany(sql){
      const time = +new Date();
      for (let i in sql) {
        sql[i].createTime = sql[i].createTime || time;
        sql[i].updateTime = sql[i].updateTime || time;
      }

      try {
        const res = await model[tableName].create(sql);
        return {
          code: 200,
          data: res,
          msg: 'success'
    ...
    async create(sql){
      const time = +new Date();
      sql.createTime = sql.createTime || time;
      sql.updateTime = sql.updateTime || time;
      try {
        const res = await model[tableName].create(sql);
        return {
          code: 200,
    ...
    async update(sql, newDate, params = {}) {
      newDate.updateTime = newDate.updateTime || +new Date();
      if (params.upsert) {
